...
local currentTextureSettings = textureTypes["male"]
local textureId = -1
...
-- visibility:                  0 or 1
-- tx_id:                       from 1 till maximum of id for current ovelay (watch table "overlaysInfo". for example "hair" have maximum 4 ids)
-- tx_normal:                   0
-- tx_material:                 0
        ...
        if v.name==name then
            ...
            if visibility ~= 0 then
                ...
                if tx_color_type == 0 then
                    v.palette = colorPalettes[palette_id][1]
...
Citizen.CreateThread(function()
    while true do
        ...
        if is_overlay_change_active then  
            ...
            --local componentHash  = Citizen.InvokeNative(0x77BA37622E22023B, ped, 4, false)
            ...
            textureId = Citizen.InvokeNative(0xC5E7204F322E49EB,currentTextureSettings.albedo, currentTextureSettings.normal, currentTextureSettings.material);  -- create texture
            for k,v in pairs(overlayAllLayers) do
...
-- arg[3] tx_id:                       from 1 till maximum of id for current ovelay (watch table "overlaysInfo". for example "hair" have maximum 4 ids)
...
function setOverlayData(pedHandle, data)
    ...
    print(data.compType, data.skinTone, data.offsetModel)

    if data.skinTone then

        skinTone = data.skinTone

        local pedIsMale = IsPedMale(ped)
...
function setOverlaySelected(data)  
    ...
    local overlayNameType = data.compType
    local offset = data.offsetModel
    local offsetVariation = data.offsetVariation
    local overlayOpacity = data.opacity
    ...
    if offsetVariation >= 1 then
        palette_id = offsetVariation
    end

    if overlayOpacity >= 0.0 then
    ...
    for i = 1, #overlayTypes do
        if overlayNameType == overlayTypes[1] then
            tx_color_type = 1
        end
    end
